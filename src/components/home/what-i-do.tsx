...
export const WhatIDo = () => (
  <section className="section">
    <div className="shell-wide text-center">
      <Reveal>
        <p className="text-xl leading-relaxed text-paper md:text-2xl">{whatIDo.lead}</p>
      </Reveal>
        ...
        {whatIDo.items.map((item, index) => (
          <Reveal key={item.title} delay={index * 0.06}>
            <div className="mx-auto max-w-md">
              ...
              <p className="mt-3 text-lg leading-relaxed text-paper-dim md:text-xl">{item.text}</p>
            </div>
          </Reveal>
        ))}
      </div>
